...
import { COMMUNITY, MEMENTO_RAZROO_ID_VS_CODE_TOKEN, MEMENTO_RAZROO_ORG_ID, MEMENTO_RAZROO_USER_ID, RAZROO_DEV_URL } from "../../constants";
...
export function createScaffold(vscode, pathId: string, recipeId: string, path: string, context, isProduction: boolean, scaffoldId: string, packageJsonParams){
  ...
  const razrooStepURL = `${isProduction ? 'https://razroo.com' : RAZROO_DEV_URL}/${orgId}/${pathId}/${recipeId}/${scaffoldId}`;
